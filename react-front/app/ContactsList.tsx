...
// @ts-ignore
export default function ContactsList({contacts, handleClick, tunnelDispatch}: { ContactViewModel, any, any }) {
...
// @ts-ignore
    ...
    const formsList = contacts.map((contactvm: ContactViewModel) => {
        ...
        return <RecordForm key={`form${contactvm.contact.uuid}`} contact={contactvm.contact}
                           tunnelDispatch={tunnelDispatch}></RecordForm>
    });
...
export function ContactsRecord({contactvm, handleClick}) {
    ...
    return <>
        ...
        <td><EditContactButton redirectionUrl={`/edit/${contact.uuid}`} contactvm={contactvm}
...
export function EditContactButton({redirectionUrl, contactvm, onClick}) {
...
export function RecordForm({contact, tunnelDispatch}) {
    ...
    const useSubmitForm = (
        url: string,
        fields: string[],
    ): [React.SyntheticEvent<HTMLFormElement>, string, []] => {
        ...
        const fetchData = async (formData) => {
            tunnelDispatch(FormStatus.Pending.toString(), null);
            ...
            try {
                ...
                setData(data);
                tunnelDispatch(FormStatus.Ok.toString(), null);
            } catch (err) {
                setData(err);
                tunnelDispatch(FormStatus.Failed.toString(), null);
            }
        };
